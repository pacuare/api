...
async def open(email: str):
    async with await psycopg.AsyncConnection.connect(settings.get().database_url_base + '/' + get_user_database(email)) as conn:
        yield conn
...
async def create_user_db(email: str, settings: settings.Settings, refresh: Literal['none', 'refresh', 'recreate'] = 'none'):
    db_name = get_user_database(email)
