...
async def generate_code(email: str):
    has_user: bool = await db.query_one(
        "select (count(*) > 0) from AuthorizedUsers where email=%s", (email,)
    )
...
async def verify(
    ...
    response: Response,
    settings: Annotated[settings.Settings, Depends(settings.get)],
    return_to: Annotated[str | None, Form()],
):
    expected_code: str = await db.query_one(
        "select code from LoginCodes where email=%s", (email,)
    )
    ...
    if expected_code.lower() == code.lower():
        ...
        resp.set_cookie(
            "auth_status",
            enc.f.encrypt(bytes(email, "utf-8")).hex(),
...
async def access_level(email: Annotated[str, Depends(require_user)]) -> AuthAccess:
    ...
    return AuthAccess(email=email, access_level="full" if full_access else "restricted")
...
async def logout(
    ...
    response: Response,
    settings: Annotated[settings.Settings, Depends(settings.get)],
    return_to: Annotated[str, Query(alias="return")] = "/",
):
    resp = response
...
async def generate_key(description: str, email: Annotated[str, Depends(require_user)]):
    (key, id, desc, created_on) = await db.query_one(
        "insert into APIKeys (email, description) values (%s, %s) returning (key, id, description, to_char(createdOn, 'YYYY-MM-DD'))",
        (email, description),
    )
...
async def delete_key(id: int, email: Annotated[str, Depends(require_user)]) -> int:
    async with db.pool.connection() as conn:
        await conn.execute(
            "delete from APIKeys where id = %s and email = %s", (id, email)
        )
    return id
