...
async def subdomain_cors(request: Request, call_next):
    ...
    if (
        ...
        and request.headers["Origin"].endswith(".pacuare.dev")
...
@app.get("/")
async def index(
    request: Request, user: Annotated[str | None, Depends(get_user)] = None
):
    return templates.TemplateResponse(
        ...
        {
            "user": user,
            "full_access": (await access_level(user)).access_level == "full"
            if user
            else None,
            "versions": ["v2", "v1"],
        },
...
def login_page(
    request: Request,
    user: Annotated[str | None, Depends(get_user)] = None,
    ...
    return templates.TemplateResponse(
        request,
        "login.html",
        {"return_to": return_to, "error": error, "embedded": embedded},
    )
...
def account_page(
    ...
    return templates.TemplateResponse(
        ...
        "account.html",
        {
            ...
            "api_keys": [
                {"id": key[0], "description": key[1], "createdOn": key[2]}
                for key in api_keys
            ],
            "embedded": embedded,
        },
...
def query_page(
    ...
    return templates.TemplateResponse(
        request,
        "query.html",
        {"user": user, "full_access": access.access_level == "full"},
    )
